...
def groundingdino_predict(
    ...
    def get_grounding_output(model, image, caption, box_threshold):
        ...
        print(f"filter result has {len(logits_filt)} boxes." )
        print(f"boxes: {boxes_filt}")
        print(f"logits_filt: {logits_filt}")
        tokenizer = model.tokenizer
        tokenized = tokenizer(caption)

        phrases = [
            get_phrases_from_posmap(logit > box_threshold, tokenized, tokenizer).replace('.', '')
            for logit
            in logits_filt
        ]
        return boxes_filt.cpu(),logits_filt.max(dim=1)[0],phrases

    dino_image = load_dino_image(image.convert("RGB"))
    boxes_filt,logits,phrases = get_grounding_output(
    ...
    return boxes_filt,logits,phrases
